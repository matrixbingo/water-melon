...
const StatusEnable: FC<CustomerGroupEnableProps> = ({ record: { name, isActive, code }, disabled }) => {
  ...
  const title = (
    <>
      确定将[ {name} ]状态修改为<span style={{padding: '0 2px', color: '#f5222d' }}>{state ? '停用' : '启用'}</span>吗？
    </>
  );
